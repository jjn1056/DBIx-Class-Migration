...
set the C<sandbox_type> flag.  Also, you should add L<Test::mysqld> to your
...
Assuming you get MySQL properly installed, lets build a sandbox:
    ...
    $dbic-migration -Ilib -Tmysql status
    Schema is 3
...
Please kind in mind that currently you can't run migration commands on an already
running database.

You'd install the database and fixtures like so:

    dbic-migration -Ilib -Tmysql install
    dbic-migration -Ilib -Tmysql populate
