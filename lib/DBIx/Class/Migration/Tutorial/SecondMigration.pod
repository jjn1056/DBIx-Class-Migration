package DBIx::Class::Migration::Tutorial::SecondMigration;
...
some geolocation information, in this case a Country that can be associated
with the Artist.  Right now we are just going to care about Artists that can
...
Oe, we got a bunch more files.  You can see that we've created a new
...
database using L<SQLT> and try to suggest some DDL that would do the job.  You
should treat this initial C<001-auto.sql> file as a suggestion and as a guide.
You will need to make changes to it based on your data change need (L<SQLT>
...
good judegment, and you might need to involve your DBA to help you craft a
...
So this is going to be the third step for this upgrade.  Open the file in your
text editor and enter this code:
...
that file (otherwise when later you run the migration, you will end up doing
more changes and get some errors)
...
You are done with the upgrade, lets move on to make new fixtures.

=head1 Customize Fixtures and Serializing Fixtures

When you C<prepare>d the new version, in addition to creating the database
...
This will dump all the countries, and you will be able to load them later.
