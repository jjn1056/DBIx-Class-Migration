package DBIx::Class::Migration::Tutorial::FirstMigration;
...
creating and use migrations for.  Typically this will be your subclass of
...
Since this is the first version, we won't create any upgrades or downgrade
migrations, since that makes little sense.  Okay, lets see what we now have
...
So Lets review.  We created a default fixture configuration that just serializes
...
C<001-auto-_VERSION.sql> which is DDL for the default database (SQLite) to create
the meta table that L<DBIx::Class::DeploymentHandler> uses to keep track of the
version history for your deployments.  We also create a full DDL (data description
language) for the tables that make up your application.  In this case we have one
table for each of the Artist, CD and Track Result classes.  
...
If your read the L<DBIx::Class::DeploymentHandler> docs, you know that in
...
At this point I'd say you have the minimum setup for being about to do real
database development.

=head1 NEXT STEPS

Proceed to L<DBIx::Class::Migration::Tutorial::>.
