package DBIx::Class::Migration::Tutorial::Testing;
...
and with general Perl testing procedures
...
default it creates a SQlite in memory database) which reflects the current 
schema and then we populate the C<all_tables> fixtures using the utility class
L<DBIx::Class::Migration::Population> which is part of the <DBIx::Class::Migration>
distribution.  Before we can run the test case, we need to update our C<dist.ini>
...
Proceed to L<DBIx::Class::Migration::ThirdMigration>
