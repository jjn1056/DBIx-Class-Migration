...
we've been using all along.  If you instead wanted to use the MySQL sandbox,
you can look at the C<connect> info in C<share/musicbase-schema/bin/use> to
help you develop that.  For our example, the Mysql sandbox would look like
this:
...
If you looked carefully, you can see we are also specifying the C<sandbox_class>
in this example.  You'd do something like this if you wanted to use a sandbox
other than the default sqlite.  In this case we are going to use Mysql as the
target sandbox for our migrations.  See L<DBIx::Class::Migration::MySQLSandbox>
for more documentation on the MySQL sandbox.<D-d>
...
Additionally, and just to show yousome of the features of L<Test::DBIx::Class>
we will create two database 'slave' replicants using MySQL native replication.
If you plan to use replication in production, as many of you do, I highly
recommend running tests in a replicated setup, to make sure your code is
replication safe.
