...
L<https://github.com/jjn1056/Maker> if you want).  I've decided I'd rather not
...
directory (contains your Makefile.PL or dist.ini, and the lib and t directories
for example) where we will create migrations.  At this time we can't
...
Unless you specify a <--target_dir> the application will default to putting all
your migration files in the C</share> directory of your application root.
Although we can automatically create most of the subdirectories, we can't right
now also automatically create a missing C</share> directory.  You need to create
it:

    mkdir share

for example does the trick on unix-like systems.
