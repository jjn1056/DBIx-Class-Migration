package DBIx::Class::Migration::FAQ;
...
=head1 Is there an easy way to automatically install the sandbox when starting Catalyst?

There's a few approaches.  If you followed the tutorial, you could substitute
C<share/etc/musicbase_web.pl> with something like the following:
...

    {
      ...
      'Model::Schema' => {
        schema_class => 'MusicBase::Schema',
        connect_info => [
          sub { $migration->schema->storage->dbh },
        ],
      },
      'View::HTML' => {
...
What this would do is when L<Catalyst> starts it will connect to a database
(in this case the default Sqlite Sandbox, creating that as needed) and will
check if there is no installed version.  If not, we install the database and
install some fixtures.

Again, this is probably abusing configuration and ideally we'd see some sort
of trait on L<Catalyst::Model::DBIC::Schema> to do this.  But for now it is
a working option.

One thing nice with this approach is how easy it is to move to a different
sandbox database.  For example, to use the MySQL sandbox instead you'd make
